...
export default function SalonReviews({ salonId, salonName }: Props) {
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-linear-to-br from-noir-500/[0.08] to-white/[0.02] p-4 sm:p-6 backdrop-blur-lg shadow-xl space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center w-full justify-between">
          <h3 className="text-white font-one font-semibold tracking-widest text-base sm:text-lg">
      ...
      <div className="space-y-2">
        ...
        <input
          ...
          className="w-full bg-white/5 border border-white/20 rounded-2xl px-2.5 py-1.5 text-white text-xs focus:outline-none focus:border-tertiary-400"
        />
        <textarea
          value={form.comment}
      ...
      <div className="space-y-3">
        {loading ? (
          ...
          reviews.map((r) => (
            <div
              key={r.id}
              className="border border-white/10 rounded-2xl p-3 bg-white/5 space-y-2"
            >
              <div className="flex items-center justify-between">
                ...
                {r.isVerified && (
                  <span className="text-emerald-300 text-xs font-one bg-emerald-500/10 border border-emerald-400/30 px-1.5 py-0.5 rounded-full">
                    ✓ Vérifié
                  </span>
                )}
              </div>
              {r.title && (
                <p className="text-white font-semibold text-xs font-one tracking-widest">{r.title}</p>
              )}
              {r.comment && (
                <p className="text-white/80 text-xs leading-relaxed font-one">
                  {r.comment}
                </p>
              )}
              ...
              {r.salonResponse && (
                <div className="mt-2 pt-2 border-t border-white/10">
                  <div className="bg-linear-to-br from-tertiary-500/10 to-tertiary-400/5 rounded-2xl p-2.5 border border-tertiary-500/20">
                    <div className="flex items-center gap-1.5 mb-1.5">
                      <div className="w-5 h-5 rounded-full bg-tertiary-500/30 flex items-center justify-center">
                      ...
                      {r.salonRespondedAt && (
                        <span className="text-white/40 text-xs ml-auto font-one">
                    ...
                    <p className="text-white/90 text-xs leading-relaxed font-one">
                      {r.salonResponse}
                    </p>
                  </div>
                </div>
      ...
      <div className="flex items-center gap-2 pt-2">
        <button
          ...
          className="cursor-pointer px-2.5 py-1.5 bg-white/5 border border-white/15 text-white/80 rounded-2xl text-xs font-one disabled:opacity-40"
        >
          Préc
        </button>
        <span className="text-white/70 text-xs font-one">Page {page}</span>
        <button
          onClick={() => load(page + 1)}
