  ...
  return (
    <Section title="Choisir la prestation">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {prestations.map((p) => {
          const isSelected = prestation === p;
          const isProject = p === "PROJET";
          ...
          return (
            ...
            >
              ...
              {isProject && isSelected && (
                <div className="mt-2 rounded-xl border border-white/15 bg-white/5 px-2 py-1.5">
                  <p className="text-[11px] text-white/75 font-one leading-relaxed">
                    Durée : {projectAppointmentDurationMinutes} min
                  </p>
                  <p className="text-[11px] text-white/75 font-one leading-relaxed">
                    Prix : {projectAppointmentIsFree ? "Gratuit" : `${projectAppointmentPrice}€`}
                  </p>
                </div>
              )}
